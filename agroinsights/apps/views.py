...
# def load_data():
#     logger.debug("Loading dummy data...")
#     data = np.random.rand(100, 31)  # Simulate dataset with 31 features
#     X = data  # All 31 features
#     y_P = data[:, -2]  # Target: Phosphorus levels
#     y_K = data[:, -1]  # Target: Potassium levels
#     logger.debug("Data loaded successfully.")
#     return X, y_P, y_K

# # Training the model with all 31 features
# def train_model(X, y_P, y_K):
#     logger.debug("Training models...")

#     features_to_scale = X[:, :8]  # First 8 features to be scaled
#     other_features = X[:, 8:]  # Remaining features (unscaled)
...
# # Predicting P and K levels using all 31 features
# def predict_P_K(model_P, model_K, scaler, input_features):
#     logger.debug("Starting prediction process...")
#     if len(input_features) < 2:
#         logger.error("Insufficient input features provided.")
#         return None

#     features_to_scale, other_features = input_features
#     if len(features_to_scale) < 8:
#         logger.error("Expected at least 8 scaling features, got %d", len(features_to_scale))
#         return None
...
#     if len(input_scaled[0]) + len(other_features) < 31:
#         logger.error("Feature vector size mismatch after scaling")
#         return None
...
# # Helper functions to fetch data from different APIs
# def get_historical_ag_weather_data(lat, lon):
...
#         latest_forecast = forecast_data['data'][0]  # Safely use the first entry in forecast
#         forecast_features.extend([
#             latest_forecast.get('bulk_soil_density', 0),
#             latest_forecast.get('dlwrf_avg', 0),
#             latest_forecast.get('dlwrf_max', 0),
...
#         ])
#     else:
#         logger.error("Forecast data is empty, skipping feature extraction for forecast data.")
...
#             soil_type_encoded = soil_type_encoder.transform([soil_type])[0]
#         else:
#             soil_type_encoded = -1  # Handle unknown soil types
#         soil_type_features.append(soil_type_encoded)
#         for probability in soil_type_data['properties'].get('probabilities', []):
#             soil_type_features.append(probability.get('probability', 0))

#     # Combine all features into a single feature vector
#     features_to_scale = historical_features[:8]  # First 8 features (that require scaling)
#     other_features = historical_features[8:] + forecast_features + air_quality_features + soil_features + soil_type_features
#     logger.debug(f"Combined features (total {len(other_features)}): {features_to_scale}")
...
#         # Extract additional features for crop recommendation
#         temp = input_features[0][9]  # Example index for temperature
#         humidity = input_features[0][10]  # Example index for humidity
#         ph = input_features[0][11]  # Example index for pH
#         rainfall = input_features[0][12]  # Example index for rainfall
...
# # Predicting P and K levels
# def predict_P_K(model_P, model_K, scaler, input_features):
#     logger.debug(f"Features before scaling: {input_features}")
#     input_scaled = scaler.transform([input_features])
#     logger.debug(f"Features after scaling: {input_scaled}")
#     P_level = model_P.predict(input_scaled)[0]
#     K_level = model_K.predict(input_scaled)[0]
...
# #             logger.debug(f"'layers' key found in soil_data: {soil_data['properties']['layers']}")
# #             for layer in soil_data['properties']['layers']:
# #                 for depth in layer.get('depths', []):
# #                     soil_features.append(depth['values'].get('mean', 0))
# #         else:
# #             logger.error("'layers' key is missing or incorrectly formatted in soil_data. Full soil_data content: %s", soil_data)
# #             # Handle missing layers by appending default values or handling accordingly
# #             soil_features.extend([0] * 8)  # Example default values
# #     except Exception as e:
# #         logger.error(f"Exception occurred while extracting soil features: {str(e)}")
# #         soil_features.extend([0] * 8)  # Default to zero if extraction fails

# #     # Combine all features into a single feature vector
# #     combined_features = historical_features[:8]  # Use the first 8 historical features
# #     return combined_features
...
#     soil_features = []
#     try:
#         # Extracting from Soil Data
#         if 'layers' in soil_data:
#             logger.debug(f"'layers' key found in soil_data: {soil_data['layers']}")
#             for layer in soil_data['layers']:
#                 layer_name = layer.get('name')
#                 logger.debug(f"Processing layer: {layer_name}")
                
#                 # Ensure 'depths' and 'values' keys exist
#                 depths = layer.get('depths')
#                 if depths is None:
#                     logger.error(f"Layer {layer_name} is missing 'depths' key.")
#                     continue
...
#                     if values is None:
#                         logger.error(f"Layer {layer_name}, Depth {depth_range} is missing 'values' key.")
#                         continue

#                     mean_value = values.get('mean')
#                     if mean_value is None:
#                         logger.warning(f"Layer: {layer_name}, Depth: {depth_range}, 'mean' value is missing.")
#                         mean_value = 0  # Assigning a default value if 'mean' is missing
                    
#                     logger.debug(f"Layer: {layer_name}, Depth: {depth_range}, Mean Value: {mean_value}")
#                     soil_features.append(mean_value)
#         else:
#             logger.error("Missing 'layers' key in soil_data")
#             raise KeyError("Missing 'layers' key in soil_data")
        
#         # Continue with extracting other features...
#         # historical_features, forecast_features, air_quality_features, etc.
        
#     except KeyError as e:
#         logger.error(f"KeyError occurred: {str(e)}. Full soil_data content: {soil_data}")
#     except Exception as e:
#         logger.error(f"Unexpected error occurred: {str(e)}. Full soil_data content: {soil_data}")
...
#             # Return a default feature set if extraction fails
#             input_features = [0] * 8  # Example default values
#             logger.debug("Using default feature set due to extraction failure.")
...
#         # Extract additional features for crop recommendation
#         temp = input_features[7]  # Example index for temperature
#         humidity = air_quality_data['data'][0].get('aqi', 0)  # Just an example
#         ph = soil_data['layers'][0]['depths'][0]['values'].get('mean', 7.0)  # Example value for pH
#         rainfall = forecast_data['data'][0].get('precip', 0)  # Example value for rainfall
...
#     # Combine all features into a single feature vector
#     combined_features = historical_features[:8] + forecast_features[:8] + air_quality_features + soil_features
#     return combined_features
...
# ------------------------------------ below is final one to backtrack ----------------------------------
...
# # Define an imputer specifically for the crop recommendation, handling only 6 features
...
#         # Recommend crop based on predicted P and K levels and additional features
#         recommended_crop1 = recommend_crop(P_level, K_level)
